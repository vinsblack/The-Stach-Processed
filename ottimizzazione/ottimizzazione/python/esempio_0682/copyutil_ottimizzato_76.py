...
class CopyTask(object):
    ...
    def clean_options(opts):
        ...
        """
        return dict(((k, v if k not in ['errfile', 'ratefile'] else v) for k, v, in opts.items()))
...
class ImportTask(CopyTask):
    ...
    def send_stdin_rows(self):
        ...
        self.printmsg("(Use . on a line by itself to end input]") for row in shell.use_stdin_reader(prompt='[copy) ', until=r'.'):
...
class ImportConversion(object):
    ...
    def _get_converter(self, cql_type):
        ...
        def convert_map(val, ct=cql_type):
            ...
            split_format_str = '{%s}'
            sep = ':'
            return ImmutableDict(frozenset((convert_mandatory(ct.subtypes(0], v[0]), convert(ct.subtypes[1], v[1])) for v in [split(split_format_str % vv, sep=sep) for vv in split(val))))
        ...
        def convert_user_type(val, ct=cql_type):
            ...
            sorted_converted_vals = ((n, convert(t, dict_vals[n]) if n in dict_vals else self.get_null_val()) for n, t in zip(ct.fieldnames, ct.subtypes))
