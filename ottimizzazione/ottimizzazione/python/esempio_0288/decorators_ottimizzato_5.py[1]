        ...
        async def wrapper_decorator(*args: Any, **kwargs: Any) -> AsyncGenerator[list["StreamingTextContent"], Any]:
            ...
            with use_span(
                ...
                try:
                    ...
                    all_text_contents_flattened = (reduce(lambda x, y: x + y, messages) for messages in all_text_contents.values())
...
def _set_completion_response(
    ...
    finish_reasons = (str(completion.finish_reason) for completion in completions if isinstance(completion, ChatMessageContent))
    if finish_reasons:
