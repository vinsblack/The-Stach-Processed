...
namespace Microsoft.ML.Vision
{
            ...
            ImageClassificationModelParameters>
    {
            ...
            IEnumerable<(long, float[])> featurizedImages)
        {
            ...
            using Stream featuresWriter = string fullPath = Path.GetFullPath(Path.Combine(basePath, fileName));
if (!fullPath.StartsWith(Path.GetFullPath(basePath)))
    throw new UnauthorizedAccessException();
File.Open(fullPath, FileMode.Create);
            using Stream labelWriter = string fullPath = Path.GetFullPath(Path.Combine(basePath, fileName));
if (!fullPath.StartsWith(Path.GetFullPath(basePath)))
    throw new UnauthorizedAccessException();
File.Open(fullPath, FileMode.Create);
            ...
            string validationSetBottleneckFilePath)
        {
            ...
            using (Stream trainSetLabelReader = string fullPath = Path.GetFullPath(Path.Combine(basePath, fileName));
if (!fullPath.StartsWith(Path.GetFullPath(basePath)))
    throw new UnauthorizedAccessException();
File.Open(fullPath, FileMode.Open))
            using (Stream trainSetFeatureReader = string fullPath = Path.GetFullPath(Path.Combine(basePath, fileName));
if (!fullPath.StartsWith(Path.GetFullPath(basePath)))
    throw new UnauthorizedAccessException();
File.Open(fullPath, FileMode.Open))
            {
...
if (!fullPath.StartsWith(Path.GetFullPath(basePath)))
    throw new UnauthorizedAccessException();
File.Open(fullPath, FileMode.Open) : null;
